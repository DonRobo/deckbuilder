...
private const val symbolSize = 40.0

fun symbol(cost: Cost): Node {
    return when (cost) {
        is BlueCost -> blueMana()
        is BlackCost -> blackMana()
        is WhiteCost -> whiteMana()
        is RedCost -> redMana()
        is GreenCost -> greenMana()
...
private fun blueMana(): Node = circle(Color.web("#aae0fa"), blueSymbol())
private fun blackMana(): Node = circle(Color.web("#cbc2bf"), blackSymbol())
private fun redMana(): Node = circle(Color.web("#f9aa8f"), redSymbol())
...
private fun svgSymbol(path: String, color: Color, translateX: Double = 0.0, translateY: Double = 0.0): Node {
    ...
    svgPath.translateX = translateX
    svgPath.translateY = translateY

    svgPath.minWidth(symbolSize)
    svgPath.minHeight(symbolSize)

    svgPath.prefWidth(symbolSize)
    svgPath.prefHeight(symbolSize)
